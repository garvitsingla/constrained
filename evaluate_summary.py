...
# ─────────────────────────────────────────────────────────────────────────────
# 1. Load Constrained RL Baseline
# ─────────────────────────────────────────────────────────────────────────────
...
print(f"\n{'='*65}")
print(f"Starting Summary Comparison for {env_name}")
...
for dt, dc in DELTA_PAIRS:
    clamaml_ckpt = f"lang_model/lang_{env_name}_dt{dt}_dc{dc}_{nc}c.pth"
    ...
    for c_room, c_dists in configs:
        ...
        for mission in test_tasks:
            ...
            theta_prime = get_clamaml_params(
                mission, policy_clamaml, encoder, m_adapter, c_adapter, dt, dc
            )
            for ep in range(n_episodes):
                env.reset_task(mission)
                s, ok, v = evaluate_policy(env, policy_clamaml, params=theta_prime, seed=ep_seeds[ep])
                results_clamaml['steps'].append(s)
                results_clamaml['successes'].append(ok)
                results_clamaml['viols'].append(v)
            ...
            if crl_ready:
                for ep in range(n_episodes):
                    ...
                    results_crl['steps'].append(s)
                    results_crl['successes'].append(ok)
                    results_crl['viols'].append(v)
            ...
            if random_ready:
                for ep in range(n_episodes):
                    ...
                    results_random['steps'].append(s)
                    results_random['successes'].append(ok)
                    results_random['viols'].append(v)

    # ── Logging Row to Excel for this Delta configuration ───────────────────
    avg_row = [dt, dc]
    ...
    # Check if (dt, dc) already exists in column A and B of sheet
    row_idx = None
    ...
    print(f"Done dt={dt}, dc={dc} | C-LAMAML SR: {sr_clamaml:.1f}% | CRL SR: {sr_crl:.1f}%")

# Save workbook
wb.save(xlsx_path)
print(f"\nAll summary results saved/appended → {xlsx_path} (sheet: '{sheet_name}')")
