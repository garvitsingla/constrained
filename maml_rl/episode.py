...
class BatchEpisodes(object):
    ...
    @property
    def cost_returns(self):
        if self._cost_returns is None:
            self._cost_returns = torch.zeros_like(self.costs)
            cost_return = torch.zeros((self.batch_size,), dtype=torch.float32, device=self.costs.device)
            for i in range(len(self) - 1, -1, -1):
                cost_return = self.gamma * cost_return + self.costs[i] * self.mask[i]
                self._cost_returns[i] = cost_return
        return self._cost_returns
